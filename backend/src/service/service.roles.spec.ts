...
describe('Service management RBAC (RolesGuard)', () => {
  const reflector = {
  ...
  it('allows OWNER to mutate services', () => {
    expect(guard.canActivate(makeContext(UserRole.OWNER))).toBe(true);
  });

  it('allows ADMIN to mutate services', () => {
  ...
  it('forbids STAFF from attach-resource mutate roles', () => {
    (reflector.getAllAndOverride as jest.Mock).mockReturnValue([
      UserRole.OWNER,
      UserRole.ADMIN,
    ]);
  ...
  it('forbids CUSTOMER from attach-resource mutate roles', () => {
    expect(() => guard.canActivate(makeContext(UserRole.CUSTOMER))).toThrow(
      ForbiddenException,
    );
  });
